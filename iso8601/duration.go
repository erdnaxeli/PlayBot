// Package iso8601 provides fonctions to parse ISO 8601 formatted duration.
package iso8601
...
// InvalidDurationError is returned when a duration cannot be parsed.
type InvalidDurationError struct {
	// Duration is the invalid string.
	Duration string
}

func (err InvalidDurationError) Error() string {
...
func ParseDuration(duration string) (time.Duration, error) {
	...
	if groups == nil {
		return 0, InvalidDurationError{duration}
	}
